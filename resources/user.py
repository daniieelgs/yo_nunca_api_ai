...
from schema import UserSchema, UserTokensSchema, UserLoginSchema, UserCategories, UserRecomendQuestion
...
@blp.route('/settings/categories')
class UserCategories(MethodView):
    
    @jwt_required(refresh=True)
    @blp.response(200, UserCategories)
    def get(self):
        user = UserModel.query.get_or_404(get_jwt_identity())
    ...
    def put(self, data):
        ...
        try:
            ...
            traceback.print_exc()
            abort(500, message = str(e) if DEBUG else 'Could not update categories list.')
            
        return {}
    
@blp.route('/settings/recomend_question')
class UserRecomentQuestion(MethodView):
    @jwt_required(refresh=True)
    @blp.arguments(UserRecomendQuestion)
    @blp.response(204)
    def post(self, data):
        ...
        examples: list = _settings["examples"]
        
        examples.append(data["question"])
        ...
        try:
            ...
            traceback.print_exc()
            abort(500, message = str(e) if DEBUG else 'Could not update recomended questions.')
